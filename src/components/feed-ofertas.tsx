...
export default function FeedOfertas({ userId, preferenciasIniciales }: FeedProps) {
  ...
  const buscarOfertas = useCallback(async () => {
    ...
    if (busqueda.trim().length > 0) {
      ...
      // Si no hay búsqueda, mostramos ofertas afines a las escuelas de interés
      // Supabase no soporta filtro array intersect directo con or fácilmente sin rpc, 
      // así que para MVP mostramos todas y el egresado buscará.
    }
